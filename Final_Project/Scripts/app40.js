...
app.filterProps = function(e) {
    ...
    var a = [],
        t = $("#searchContent").data("form"),
        n = "";
    switch (t) {
        case "map-view":
            return announcesMap.loadData(null, e), !1;
        case "search":
            var o = { area: "ایران", sales_type: "همه", type: "املاک", size: null },
                s = globals.types;
    ...
    var t = decodeURI(location.href).replace(base_url, ""),
        n = app.convertFormValues(e);
        ...
        on: {
            slideChange: function() {
                ...
                if ($(e.find("img")).data("src")) {
                    var a = $(e.find("img")).clone(),
                        t = a.data("src"),
                        n = e.find("a").length ? e.find("a") : e;
...
searchModule.search_element = $(".search-location"), searchModule.searchBox = $(".search-box"), searchModule.resultBox = $(".result-box"), searchModule.searchInput = $(".search-input"), searchModule.activeResult = ".search-result-item.active", searchModule.timer_timeout = "", searchModule.results = [], searchModule.placeholder = "", searchModule.allowReload = !0, searchModule.search = function(e) {
    var a = !0 === e.data("area") ? "area-location/" : "search-location/",
        t = e.val();
    ...
    var a = $(".filter_form").serializeArray(),
    ...
    var s = Object.keys(o),
        i = $(".searched .container");
    if (s.length)
        for (var r in i.removeClass("hidden"), i.html(""), o) {
...
loginModule.loginChooseContainer = $(".login-chooser"), loginModule.loginContainer = $(".login-container"), loginModule.registerCode = $(".register-code"), loginModule.resetContainer = $(".reset-password"), loginModule.registerContainer = $(".register-container"), loginModule.baseContainer = $(".auth-container"), loginModule.timer = 90, loginModule.actionAfterLogin = "", loginModule.showMessage = function(e, a) {
    ...
    if (member = e.object, loginModule.checkLogin(), requests.get("auth-menu", function(e) {
            $("#auth_content").html(e)
        }, function(e) {}), $("#authModal").modal("hide"), loginModule.actionAfterLogin.indexOf("fav") > -1) {
    ...
    swal({
        ...
        e.value && (member = "", requests.get("logout", {}, function(e) {
            location.reload()
        }, function(e) {}), $(".modal_favorite").modal("hide"), loginModule.checkLogin())
    })
    ...
    $(".modal-body .back").click(function() {
        ...
        e.preventDefault();
        var a = $(this).data("submit"),
            t = $(this).serializeArray().reduce(function(e, a) {
...
var announceModule = {
    ...
    submitForm: function(e) {
        ...
        if (a.area_id = a.location_ids, member.hasOwnProperty("id")) {
            var o = [];
            if (announceModule.myDropzone.length && announceModule.myDropzone[0].dropzone.files.length)
                for (var s in announceModule.myDropzone[0].dropzone.files) {
...
$("div#my-awesome-dropzone").length ? announceModule.myDropzone = $("div#my-awesome-dropzone").dropzone({
    ...
    $("#IranMap svg g path").hover(function() {
        ...
        var e = $(this).attr("class"),
            a = $("#IranMap .list>ul>li>ul>li." + e + " a").html();
    ...
    if (automateHover.putImg(), $(".agency-hover-show").hover(function() {
            ...
            var e = $(".property-sec"),
                a = $(".property-box").data("link-number");
    ...
    $(".filter_form_home").submit(function(e) {
        ...
        var e = $(this).val(),
            a = $("select[name='bedroom']"),
            t = $("select[name='build_year']"),
            n = $("input[name='floor']"),
            o = $("input[name='sum_floor']");
...
$(document).ready(function() {
    ...
    $(document).on("change", "select[name=spacial_sales_type],select[name=order_agency]", function() {
        var e = $("input[name=agency_id]").val(),
            a = $("input[name=special_type]").val();
        if ($(this).parent().hasClass("mobile-agent-sort")) {
            $(this).parents(".modal").modal("hide");
            var t = $("select[name=spacial_sales_type]:checked").val(),
                n = $("select[name=order_agency]:checked").val()
...
var ratingStar = {
    ...
    show: function() {
        $(".rating-stars-show").each(function() {
            ...
            if (parseFloat(e) - t != 0) {
                if (parseFloat(e) - t >= .5)
                    for (var n = 0; n < t + 1; n++) $(a[n]).children(n).removeClass("sh-star-o").addClass("sh-star"), n == t && $(a[n]).children(n).removeClass("sh-star").addClass("sh-starh");
                if (parseFloat(e) - t < .5)
                    for (n = 0; n < t; n++) $(a[n]).children(n).removeClass("sh-star-o").addClass("sh-star")
            } else
                for (n = 0; n < parseInt(e); n++) $(a[n]).children(n).removeClass("sh-star-o").addClass("sh-star")
        })
    }
};
$(document).ready(function() {
